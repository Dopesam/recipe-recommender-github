...
# OpenAI Configuration - using modern client approach
# The API key will be set when creating the client
...
def init_database():
    ...
    # Create AI-generated recipes table
    conn.execute('''
...
@require_auth
def generate_ai_recipe_from_ingredients():
    """Generate a recipe using AI based on user ingredients and preferences"""
    data = request.get_json()
    ...
    if not user_ingredients or len(user_ingredients) < 2:
        return jsonify({'error': 'At least 2 ingredients are required'}), 400
    
    try:
        # Check if OpenAI API key is set
        api_key = os.getenv('OPENAI_API_KEY')
        if not api_key or api_key == 'your-openai-api-key-here':
            return jsonify({'error': 'OpenAI API key is not configured. Please set OPENAI_API_KEY environment variable.'}), 500
        ...
        if preferences.get('difficulty'):
            prompt_parts.append(f"The recipe should be {preferences['difficulty']} difficulty.")
        
        if preferences.get('cuisine'):
            prompt_parts.append(f"Make it {preferences['cuisine']} style cuisine.")
        
        user_prompt = " ".join(prompt_parts)
        
        # Create structured prompt for recipe generation
        system_prompt = """
You are a professional chef and recipe developer. Generate a detailed recipe based on the user's ingredients and preferences.
Return ONLY a valid JSON object with the following structure (no markdown formatting or code blocks):

{
    ...
    "health_benefits": "Health benefits of this dish",
    "ingredients": ["ingredient 1 with measurement", "ingredient 2 with measurement"],
    "steps": ["detailed step 1", "detailed step 2"]
}

Ensure ingredients include proper measurements and steps are detailed and clear.
        """
        ...
        # Convert lists to pipe-separated strings for database storage
        ingredients_str = '|'.join(recipe_data['ingredients'])
        steps_str = '|'.join(recipe_data['steps'])
        
        # Use a food-related placeholder image for AI-generated recipes
        placeholder_image = 'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=400&h=300&fit=crop'
        
        # Save to AI recipes table
        conn = get_db_connection()
        conn.execute('''
            INSERT INTO ai_recipes (
                recipe_id, name, country, origin, cuisine_type, description, image,
                prep_time, difficulty, spice_level, is_vegan, is_vegetarian, is_gluten_free,
                health_benefits, ingredients, steps, user_prompt, created_by
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            recipe_id,
            recipe_data['name'],
            ...
            recipe_data['spice_level'],
            int(recipe_data['is_vegan']),
            int(recipe_data['is_vegetarian']),
            int(recipe_data['is_gluten_free']),
            recipe_data['health_benefits'],
            ingredients_str,
            steps_str,
            user_prompt,
            session['user_id']
        ...
        formatted_recipe = {
            ...
            'health_benefits': recipe_data['health_benefits'],
...
def generate_ai_recipe():
    ...
    try:
        ...
        # Create structured prompt for recipe generation
        system_prompt = """
You are a professional chef and recipe developer. Generate a detailed recipe based on the user's request.
...
Ensure all fields are filled appropriately. For ingredients, provide clear measurements. For steps, provide detailed cooking instructions.
        """
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": f"Create a recipe for: {user_prompt}"}
            ],
            max_tokens=1500,
            temperature=0.7
        )
        ...
        # Use a placeholder image for AI-generated recipes
        placeholder_image = 'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=400&h=300&fit=crop'
        ...
        # Return the formatted recipe with AI badge
        formatted_recipe = {
            'id': recipe_id,
            'name': recipe_data['name'],
            'country': recipe_data['country'],
...
@require_auth
def get_user_ai_recipes():
    """Get AI-generated recipes for the current user"""
    conn = get_db_connection()
...
@app.route('/api/ai/recipe/<recipe_id>')
def get_ai_recipe_details(recipe_id):
    """Get detailed information for a specific AI-generated recipe"""
    conn = get_db_connection()
    recipe = conn.execute(
        'SELECT * FROM ai_recipes WHERE recipe_id = ?', 
        (recipe_id,)
    ).fetchone()
    conn.close()
    ...
    if recipe:
        ...
        return jsonify({'error': 'AI recipe not found'}), 404
...
def rate_recipe_frontend():
    ...
    # Handle AI recipes (which use string IDs)
    if str(recipe_id).startswith('ai_') or len(str(recipe_id)) > 10:
        recipe_type = 'ai'
        # Remove 'ai_' prefix if present
        if str(recipe_id).startswith('ai_'):
            recipe_id = str(recipe_id)[3:]
    ...
    if recipe_type == 'ai':
        recipe_exists = conn.execute(
            'SELECT recipe_id FROM ai_recipes WHERE recipe_id = ?', 
            (str(recipe_id),)
        ).fetchone()
    else:
        recipe_exists = conn.execute(
...
def get_user_ratings():
    ...
    ratings = conn.execute('''
        SELECT 
            ...
            rr.created_at,
            CASE 
                WHEN rr.recipe_type = 'ai' THEN ar.name
                ELSE r.name 
            END as recipe_name
        FROM recipe_ratings rr
        LEFT JOIN recipes r ON rr.recipe_id = r.id AND rr.recipe_type = 'regular'
        LEFT JOIN ai_recipes ar ON rr.recipe_id = ar.recipe_id AND rr.recipe_type = 'ai'
        WHERE rr.user_id = ?
        ORDER BY rr.created_at DESC
    ''', (session['user_id'],)).fetchall()
...
@require_auth
def ai_chat_conversation():
    """Interactive AI cooking conversation endpoint"""
    data = request.get_json()
    ...
    try:
        ...
        response = ai_assistant.chat_conversation(message, conversation_history)
...
@app.route('/api/ai/analyze-recipe', methods=['POST'])
@require_auth
def ai_analyze_recipe():
    """AI recipe analysis and improvement suggestions"""
    data = request.get_json()
    ...
    recipe_id = data['recipe_id']
    recipe_type = data.get('recipe_type', 'regular')
    
    # Get recipe details
    conn = get_db_connection()
    
    if recipe_type == 'ai' or str(recipe_id).startswith('ai_') or len(str(recipe_id)) > 10:
        recipe = conn.execute(
            'SELECT * FROM ai_recipes WHERE recipe_id = ?', 
            (str(recipe_id).replace('ai_', ''),)
        ).fetchone()
    else:
        recipe = conn.execute(
    ...
    try:
        ...
        analysis = ai_assistant.analyze_recipe(recipe_data)
...
@app.route('/api/ai/cooking-tips', methods=['POST'])
@require_auth
def ai_cooking_tips():
    """Generate cooking tips based on user query"""
    data = request.get_json()
    ...
    try:
        ai_assistant = get_ai_assistant()
        tips = ai_assistant.get_cooking_tips(query, skill_level)
        ...
        print(f"Cooking Tips Error: {e}")
        return jsonify({'error': 'Failed to generate cooking tips'}), 500

@app.route('/api/ai/substitutions', methods=['POST'])
@require_auth
def ai_ingredient_substitutions():
    ...
    ingredient = data['ingredient'].strip()
    ...
    try:
        ai_assistant = get_ai_assistant()
        substitutions = ai_assistant.suggest_substitutions(ingredient, recipe_context, dietary_restrictions)
        ...
        print(f"Substitutions Error: {e}")
        return jsonify({'error': 'Failed to generate substitutions'}), 500

@app.route('/api/ai/troubleshoot', methods=['POST'])
@require_auth
def ai_cooking_troubleshoot():
    """Get cooking troubleshooting help"""
    data = request.get_json()
    ...
    try:
        ai_assistant = get_ai_assistant()
        solution = ai_assistant.troubleshoot_cooking(problem, recipe_context)
        ...
        return jsonify({'error': 'Failed to provide troubleshooting help'}), 500
...
def ai_meal_plan():
    ...
    try:
        ai_assistant = get_ai_assistant()
        meal_plan = ai_assistant.create_meal_plan(
            days=days,
...
@app.route('/api/ai/technique', methods=['POST'])
@require_auth
def ai_cooking_technique():
    """Get cooking technique explanations"""
    data = request.get_json()
    
    if not data or 'technique' not in data:
        return jsonify({'error': 'Technique name is required'}), 400
    
    technique = data['technique'].strip()
    skill_level = data.get('skill_level', 'intermediate')
    
    if not technique:
        return jsonify({'error': 'Technique name cannot be empty'}), 400
    
    try:
        ai_assistant = get_ai_assistant()
        explanation = ai_assistant.explain_cooking_technique(technique, skill_level)
        ...
        print(f"Technique Explanation Error: {e}")
        return jsonify({'error': 'Failed to explain cooking technique'}), 500

@app.route('/api/ai/variations', methods=['POST'])
@require_auth
def ai_recipe_variations():
    ...
    recipe_id = data['recipe_id']
    recipe_type = data.get('recipe_type', 'regular')
    variation_type = data.get('variation_type', 'general')  # dietary, regional, seasonal, etc.
    ...
    try:
        ...
        # Format recipe for variation generation
        recipe_data = {
            'name': recipe['name'],
            'ingredients': recipe['ingredients'].split('|') if recipe['ingredients'] else [],
            'steps': recipe['steps'].split('|') if recipe['steps'] else [],
        ...
        variations = ai_assistant.suggest_recipe_variations(recipe_data, variation_type)
        ...
        print(f"Recipe Variations Error: {e}")
        return jsonify({'error': 'Failed to generate recipe variations'}), 500

@app.route('/api/ai/dietary-assistant', methods=['POST'])
@require_auth
def ai_dietary_assistant():
    """Personalized dietary recommendations and analysis"""
    data = request.get_json()
    
    user_profile = data.get('user_profile', {})
    query = data.get('query', '')
    
    # Get user's dietary preferences from database if not provided
    if not user_profile:
        conn = get_db_connection()
        user = conn.execute(
            'SELECT cuisine_preferences FROM users WHERE id = ?',
            (session['user_id'],)
        ).fetchone()
        conn.close()
        
        if user and user['cuisine_preferences']:
            user_profile = {
    ...
    try:
        ai_assistant = get_ai_assistant()
        recommendations = ai_assistant.provide_dietary_recommendations(user_profile, query)
        ...
        print(f"Dietary Assistant Error: {e}")
        return jsonify({'error': 'Failed to provide dietary recommendations'}), 500

@app.route('/api/ai/generate-enhanced', methods=['POST'])
@require_auth
def ai_generate_enhanced_recipe():
    """Enhanced AI recipe generation with rich context"""
    data = request.get_json()
    ...
    try:
        ...
        # Use enhanced recipe generation with context
        recipe_data = ai_assistant.generate_recipe_with_context(
            prompt=prompt,
            ingredients=context.get('ingredients', []),
            dietary_restrictions=context.get('dietary_restrictions', []),
            cuisine_preference=context.get('cuisine_preference', ''),
        ...
        # Use a food-related placeholder image
        placeholder_image = 'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=400&h=300&fit=crop'
        ...
        # Return the formatted recipe with additional AI insights
        formatted_recipe = {
            'id': recipe_id,
            'name': recipe_data['name'],
            'country': recipe_data['country'],
            ...
            'is_ai_generated': True,
            'user_prompt': prompt,
            'ai_insights': recipe_data.get('ai_insights', {})
        }
        
        return jsonify(formatted_recipe)
        
    except Exception as e:
        print(f"Enhanced Recipe Generation Error: {e}")
        return jsonify({'error': 'Failed to generate enhanced recipe'}), 500

@app.route('/api/ai/image-generate', methods=['POST'])
@require_auth
def ai_generate_recipe_image():
    """Generate recipe image using AI (placeholder for future implementation)"""
    data = request.get_json()
    ...
    try:
        ...
        # In the future, this could integrate with DALL-E or similar
        image_data = image_generator.generate_recipe_image(recipe_name, style)
        
        return jsonify({
            'success': True,
            'image_url': image_data.get('url', 'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=400&h=300&fit=crop'),
            'recipe_name': recipe_name,
            'style': style,
            'note': 'Image generation is currently in development. Using placeholder.'
        })
        
    except Exception as e:
        print(f"Image Generation Error: {e}")
        return jsonify({'error': 'Failed to generate recipe image'}), 500
